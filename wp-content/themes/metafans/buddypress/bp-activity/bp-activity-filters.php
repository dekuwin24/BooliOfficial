...
function bp_activity_truncate_entry( $text, $args = array() ) {
	...
	if ( strlen( $excerpt ) < strlen( strip_shortcodes( $text ) ) ) {
		...
		$excerpt = sprintf( '%1$s<span class="activity-read-more-custom" id="%2$s"><a href="%3$s" rel="nofollow">%4$s</a></span>', $excerpt, $id, bp_get_activity_thread_permalink(), $append_text );
	}
