...
<?php if ( ! WC()->cart->is_empty() ) : ?>
				...
				<li class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					if ( $thumbnail ) {
						if ( ! empty( $product_permalink ) ) {
							...
							echo tophive_sanitize_filter($thumbnail); // WPCS: XSS OK.
							echo '</a>';
						} else {
					...
					if ( empty( $product_permalink ) ) :
						?>
						<?php echo tophive_sanitize_filter($product_name); // WPCS: XSS OK.
						?>
					<?php else : ?>
	...
	<div class="wc-mini-cart-footer">
		<p class="woocommerce-mini-cart__total total"><?php _e( 'Subtotal', 'metafans' ); ?>: <?php echo WC()->cart->get_cart_subtotal(); ?></p>
		<p class="woocommerce-mini-cart__buttons buttons"><?php do_action( 'woocommerce_widget_shopping_cart_buttons' ); ?></p>
	</div>

<?php else : ?>

	<p class="woocommerce-mini-cart__empty-message"><?php _e( 'No products in the cart.', 'metafans' ); ?></p>
