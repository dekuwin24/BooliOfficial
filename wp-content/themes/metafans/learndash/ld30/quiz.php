...
global $post;
if ( empty( $course_id ) ) {
	$course_id = apply_filters( 'tophive/learndash/single-course/course-id', $post->ID );
}
...
$content_urls        = apply_filters( 'tophive/learndash/single-course/custom-pagination', $course_id, $lession_list, $course_quizzes_list );
$quiz_urls           = apply_filters( 'tophive/learndash/single-course/quiz/count', $course_id, $lession_list, $course_quizzes_list );
$pagination_urls     = apply_filters( 'tophive/learndash/single-course/next-prev-url', $content_urls );
$current_quiz_key    = apply_filters('tophive/learndash/single-course/custom-quiz-key', $quiz_urls );
$topics              = array();
...
<div id="learndash-content" class="container-full">
    ...
    <div class="mf-ld-single-item-grid grid">
        <?php
        if ( ! empty( $course ) ) :
                ...
                <div class="<?php echo esc_attr( learndash_the_wrapper_class() ); ?>">
                 ...
                 ?>
                    <div id="learndash-course-header" class="bb-lms-header quiz-fix">
                        <div class="ec-d-flex mf-ld-position">
                            <div class="sfwd-course-position">
                                <span class="bb-pages"><?php echo LearnDash_Custom_Label::get_label( 'quiz' ); ?> <?php echo $current_quiz_key; ?> <span class="bb-total"><?php _e( 'of', 'metafans' ); ?> <?php echo count( $quiz_urls ); ?></span></span>
                            </div>
                            <div class="sfwd-course-nav">
                                ...
                                $current             = time();
                                $expire_string       = ( $expire_date_calc > $current ) ? __( 'Expires at', 'metafans' ) : __( 'Expired at', 'metafans' );
                     ...
                     if( $show_content ):
                        ...
                        if ( $attempts_left ):
                            ...
                            learndash_get_template_part( 'modules/alert.php', array(
                                'type'      =>  'warning',
                                'icon'      =>  'alert',
                                'message' => sprintf( esc_html_x( 'You have already taken this %1$s %2$d time(s) and may not take it again.', 'placeholders: quiz, attempts count', 'metafans' ), learndash_get_custom_label_lower('quiz'), $attempts_count )
                            ), true );
                            /**
