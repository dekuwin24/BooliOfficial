...
if ( function_exists( 'is_plugin_active' ) && is_plugin_active('learndash-course-grid/learndash_course_grid.php') && isset( $shortcode_atts['course_grid'] ) && '' !== $shortcode_atts['course_grid'] ) {
	...
	$button_text = isset( $button_text ) && ! empty( $button_text ) ? $button_text : __( 'See more...', 'metafans' );
	...
	$price = $course_options && isset($course_options['sfwd-courses_course_price']) ? $course_options['sfwd-courses_course_price'] : __( 'Free', 'metafans' );
	...
	if ( $has_access && ! $is_completed && $price_type != 'open' && empty( $ribbon_text ) ) {
		...
		$ribbon_text = __( 'Enrolled', 'metafans' );
	} elseif ( $has_access && $is_completed && $price_type != 'open' && empty( $ribbon_text ) ) {
		$class .= '';
		$ribbon_text = __( 'Completed', 'metafans' );
                    ...
                    <div class="bb-course-cover">
                        <a title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="bb-cover-wrap">
							...
							if( isset( $custom_ribbon_text ) && ! empty( $custom_ribbon_text ) ) {
								echo '<div class="ld-status ld-status-progress ld-primary-background ld-custom-ribbon-text">' .sprintf( esc_html_x('%s', 'Start ribbon', 'metafans'), $custom_ribbon_text ) . '</div>';
							}
							...
							elseif( is_user_logged_in() && isset($has_access) && $has_access ) {
								...
								if ( ( $course_pricing['type'] === 'open' && $progress['percentage'] === 0 ) || ( $course_pricing['type'] !== 'open' && $has_access && $progress['percentage'] === 0 ) ) {
									...
									echo '<div class="ld-status ld-status-progress ld-primary-background">' .sprintf( esc_html_x('Start %s ', 'Start ribbon', 'metafans'), $ribbon_title ) . '</div>';
								...
								echo '<div class="ld-status ld-status-incomplete ld-third-background">' . __( 'Free', 'metafans' ) . '</div>';
								...
								echo '<div class="ld-status ld-status-progress ld-primary-background">' .
									__( 'Start ', 'metafans' ) .
									sprintf( __( '%s', 'metafans' ), LearnDash_Custom_Label::get_label( 'course' ) ) .
								'</div>';
			...
			<?php endif; ?>
                ...
                <div class="bb-card-course-details">
					...
					if( $lessons_count > 0 ) {
						...
						echo '<div class="course-lesson-count">' . __( '0 ', 'metafans' ) . sprintf( __( '%s', 'metafans' ), LearnDash_Custom_Label::get_label( 'lessons' ) ) . '</div>';
					}
					?>
