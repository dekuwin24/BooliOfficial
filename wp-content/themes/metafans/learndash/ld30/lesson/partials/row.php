...
if ( ( empty( $atts ) ) && ( ! is_user_logged_in() ) ) {
	if ( 'is_sample' === $lesson['sample'] ) {
		if ( true !== (bool) apply_filters( 'learndash_lesson_sample_access', true, $lesson['post']->ID, $course_id, $user_id ) ) {
			$atts = apply_filters( 'learndash_lesson_row_atts_sample_no_access', 'data-ld-tooltip="' . esc_html__( 'Please login to view sample content', 'metafans' ) . '"', $lesson['post']->ID, $course_id, $user_id );
		}
	}
}
                        ...
                        */
                        ...
                        if( $content_count['topics'] > 0 ): ?>
                            ...
                            <span class="ld-item-component">
                                <?php
                                    echo sprintf( esc_html__( '%s', 'metafans' ), $content_count['topics'] ) . ' ' .
                                    _n(
                                        sprintf( esc_html__( '%s', 'metafans' ), LearnDash_Custom_Label::get_label('topic') ),
                                        sprintf( esc_html__( '%s', 'metafans' ), LearnDash_Custom_Label::get_label('topics') ),
                        ...
                        if( $content_count['quizzes'] > 0 ): ?>
                            <span class="ld-item-component">
                                <?php
                                    echo sprintf( esc_html__( '%s', 'metafans' ), $content_count['quizzes'] ) . ' ' .
                                    _n(
                                        sprintf( esc_html__( '%s', 'metafans' ), LearnDash_Custom_Label::get_label('quiz') ),
                                        sprintf( esc_html__( '%s', 'metafans' ), LearnDash_Custom_Label::get_label('quizzes') ),
            ...
            */
            ...
            if( !empty($topics) || !empty($quizzes) ): ?>
                ...
                <div class="ld-expand-button ld-button-alternate" data-ld-expands="<?php echo esc_attr( 'ld-expand-' . $lesson['post']->ID ); ?>">
                    ...
                    <span class="ld-text ld-primary-color"><?php esc_html_e( 'Expand', 'metafans' ); ?></span>
