        ...
        <?php endif; ?>
            <div class="ld-progress-heading">
                <?php if( $context === 'topic' ): ?>
                    <div class="ld-progress-label"><?php echo sprintf( esc_html_x( '%s Progress', 'Placeholder: Lesson Progress', 'metafans' ), LearnDash_Custom_Label::get_label('lesson') ); ?></div>
                <?php endif; ?>
            </div>
            ...
            <div class="ld-progress-stats">
                <div class="ld-progress-percentage ld-secondary-color course-completion-rate"><?php echo sprintf( esc_html_x('%s%% Complete', 'placeholder: Progress percentage', 'metafans'), $progress['percentage'] ); ?></div>
                <div class="ld-progress-steps">
                    ...
                    if ( $context === 'course' || $context === 'focus' ):
                        ...
                        if( $course_activity && $context === 'course' ) {
	                        ...
	                        echo sprintf( esc_html_x( 'Last activity on %s', 'Last activity date in infobar', 'metafans' ), date_i18n( get_option( 'date_format' ), strtotime( $date_time_display ) ) );
                        } else {
	                        echo sprintf( __( '%s/%s Steps', 'metafans' ), $progress['completed'], $progress['total'] );
                        }
                    endif;
                    ?>
                </div>
