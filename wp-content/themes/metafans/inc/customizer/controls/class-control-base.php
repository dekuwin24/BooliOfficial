...
class Tophive_Customizer_Control_Base extends WP_Customize_Control {
	/**
	 * The control type.
	 *
	 * @access public
	...
	public function to_json() {
		...
		$this->json['_pro']           = class_exists( 'Tophive_Pro' );
		...
		if ( 'repeater' == $this->setting_type ) {
			$this->json['l10n']             = array(
				'untitled' => esc_html__( 'Untitled', 'metafans' ),
			);
	...
	function merge_items( $destination = array(), $new_array = array() ) {
		return Tophive_Customizer::merge_items( $destination, $new_array );
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'tophive-customizer-control', esc_url( get_template_directory_uri() ) . '/assets/css/admin/customizer/customizer' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_style( 'select2', esc_url( get_template_directory_uri() ) . '/assets/css/admin/select2' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_script( 'tophive-color-picker-alpha', esc_url( get_template_directory_uri() ) . '/assets/js/customizer/color-picker-alpha' . $suffix . '.js', array( 'wp-color-picker' ), false, true ); // phpcs:ignore
		...
		if ( is_null( self::$_args_loaded ) ) {
			...
			$args = array(
				...
				'is_rtl'           => is_rtl(),
				'theme_default'    => esc_html__( 'Theme Default', 'metafans' ),
				'reset'            => esc_html__( 'Reset this section settings', 'metafans' ),
				'untitled'         => esc_html__( 'Untitled', 'metafans' ),
				'confirm_reset'    => esc_html__( 'Do you want to reset this section settings?', 'metafans' ),
				'list_font_weight' => array(
					''       => esc_html__( 'Default', 'metafans' ),
					'normal' => _x( 'Normal', 'tophive-font-weight', 'metafans' ),
					'bold'   => _x( 'Bold', 'tophive-font-weight', 'metafans' ),
				),
				'typo_fields'      => tophive_metafans()->customizer->get_typo_fields(),
				'styling_config'   => tophive_metafans()->customizer->get_styling_config(),
				'devices'          => tophive_metafans()->customizer->devices,
			...
			wp_localize_script( 'tophive-customizer-control', 'Tophive_Control_Args', apply_filters( 'Tophive_Control_Args', $args ) ); // phpcs:ignore
	...
	protected function render() {
		...
		?>
		<li id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?><?php echo esc_attr($this->device) ? '  tophive--device-show tophive--device-' . esc_attr( $this->device ) : ''; ?>">
	...
	protected function render_content() {
		...
		if ( 'heading' == $this->setting_type ) {
			?>
			<div class="tophive-control--heading">
				<label>
		...
		?>
		<div class="tophive--settings-wrapper <?php echo ( esc_attr($this->label) || esc_attr($this->description) ) ? 'has-title-desc' : 'no-title-desc'; ?>">
			<?php if ( $this->label ) { ?>
			...
			if ( 'custom_html' == $this->setting_type ) {
				...
				<div class="custom_html"><?php
					echo esc_attr($this->description);  // WPCS: XSS OK.
				?></div>
				<?php
			} else {
				?>
			...
			<?php if ( 'custom_html' != $this->setting_type ) { ?>
				<div class="tophive--settings-fields<?php echo ( 'repeater' == $this->setting_type ) ? ' tophive--repeater-items' : ''; ?>"></div>
				<?php if ( 'repeater' == $this->setting_type ) { ?>
					<div class="tophive--repeater-actions">
						<a href="#" class="tophive--repeater-reorder" data-text="<?php esc_attr_e( 'Reorder', 'metafans' ); ?>" data-done="<?php _e( 'Done', 'metafans' ); ?>"><?php _e( 'Reorder', 'metafans' ); ?></a>
						<?php if ( false !== $this->addable ) { ?>
							<button type="button" class="button tophive--repeater-add-new"><?php _e( 'Add an item', 'metafans' ); ?></button>
						<?php } ?>
					</div>
				<?php } ?>
			<?php } ?>
	...
	static function before_field() {
		...
		#>
		<div class="tophive--field tophive--field-{{ field.type }} {{ field.class }} tophive--field-name-{{ field.original_name }}" data-required="{{ required }}" data-field-name="{{ field.name }}">
		<?php
	}
	...
	static function field_header() {
		...
		<div class="tophive-field-header">
			<# if ( field.label ) { #>
			<div class="tophive-field-heading">
	...
	static function field_template() {
		?>
		<script type="text/html" id="tmpl-field-tophive-text">
			<?php
			self::before_field();
			?>
			<?php echo self::field_header(); ?>
			<div class="tophive-field-settings-inner">
				<input type="{{ field.type }}" class="tophive-input tophive-only" placeholder="{{ field.placeholder }}" data-name="{{ field.name }}" value="{{ field.value }}">
			</div>
			<?php
			self::after_field();
			?>
