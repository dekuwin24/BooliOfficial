...
class Tophive_Customizer_Auto_CSS {
	 ...
	 * Get intance.
	 *
	 * @return Tophive_Customizer_Auto_CSS
	 */
	...
	function setup_shadow( $value, $format ) {
		...
		$value    = Tophive_Sanitize_Input::sanitize_color( $p['color'] );
		$color    = $value;
	...
	function setup_color( $value, $format ) {
		$value = Tophive_Sanitize_Input::sanitize_color( $value );
		if ( $format ) {
	...
	function setup_image( $value, $format ) {
		$image = tophive_metafans()->get_media( $value );
		if ( $format ) {
			if ( $image ) {
	...
	function styling( $field, $values = null ) {
		$values = tophive_metafans()->get_setting( $field['name'], 'all' );
		...
		if ( isset( $values['normal']['bg_image'] ) ) {
			$image = tophive_metafans()->get_media( $values['normal']['bg_image'] );
			if ( ! $image ) {
		...
		foreach ( tophive_metafans()->customizer->devices as $device ) {
			$css = '';
			if ( isset( $selectorCSSDevices[ $device ] ) ) {
	...
	function maybe_devices_setup( $field, $call_back, $values = null, $no_selector = false ) {
		...
		if ( isset( $field['device_settings'] ) && $field['device_settings'] ) {
			...
			foreach ( tophive_metafans()->customizer->devices as $device ) {
				$value = null;
				if ( is_null( $values ) ) {
					$value = tophive_metafans()->get_setting( $field['name'], $device );
				} else {
			...
			if ( is_null( $values ) ) {
				$values = tophive_metafans()->get_setting( $field['name'] );
			}
			if ( method_exists( $this, $call_back ) ) {
		...
		$code_array = apply_filters( 'tophive/customizer/auto_css', $code_array, $field, $this );
	...
	function typography( $field ) {
		...
		$fields      = array();
		$devices_css = array();
		foreach ( tophive_metafans()->customizer->get_typo_fields() as $f ) {
			$fields[ $f['name'] ] = $f;
		}
		...
		$devices_css = apply_filters( 'tophive/customizer/auto_css', $devices_css, $field, $this );
	...
	public function render_css( $fields = array() ) {
		...
		$css_code = apply_filters( 'tophive/auto-css', $css_code, $this );
		return $css_code;
	}
