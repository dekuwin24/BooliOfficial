...
class Tophive_Posts_Layout {
	...
	function set_args( $customizer_args = array() ) {
		$args = array(
			'layout'              => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_layout' ),
			'excerpt_type'        => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_excerpt_type' ),
			'excerpt_length'      => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_excerpt_length' ),
			'excerpt_more'        => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_excerpt_more' ),
			'more_text'           => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_more_text' ),
			'more_display'        => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_more_display' ),
			'thumbnail_size'      => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_thumbnail_size' ),
			'hide_thumb_if_empty' => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_hide_thumb_if_empty' ),
			'meta_config'         => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_meta_config' ),
			'meta_sep'            => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_meta_sep' ),
			'author_avatar'       => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_author_avatar' ),
			'media_hide'          => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_media_hide' ),
		);

		$size = tophive_metafans()->get_setting( $customizer_args['prefix'] . '_avatar_size' );
		if ( is_array( $size ) && isset( $size['value'] ) ) {
			$args['avatar_size'] = absint( $size['value'] );
		}

		$pagination = array(
			'show_paging' => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_pg_show_paging' ),
			'show_nav'    => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_pg_show_nav' ),
			'mid_size'    => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_pg_mid_size' ),
			'prev_text'   => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_pg_prev_text' ),
			'next_text'   => tophive_metafans()->get_setting( $customizer_args['prefix'] . '_pg_next_text' ),
		);
		...
		Tophive_Post_Entry()->set_config( $this->args );
	}
	...
	function item_part( $part = '', $post = null, $inner_class = '' ) {
		...
		if ( isset( $this->args[ $part . '_fields' ] ) && $n > 0 ) {
			if ( 'media' == $part && $this->has_only_field( $this->args[ $part . '_fields' ] ) ) {
				Tophive_Post_Entry()->build_fields( $this->args[ $part . '_fields' ], $post );
			} else {
			...
			case 'blog_column':
				if ( $show_media && $this->count_item_visibility( $this->args['header_fields'] ) ) {
					...
					<div class="entry-article-part entry-media">
						...
						<?php
						Tophive_Post_Entry()->build_fields( $media_fields, $post );
	...
	function blog_item( $post = null, $class = null ) {
		...
		Tophive_Post_Entry()->set_post( $post );
		/**
		 * Hook before each post
		 *
		 * @since 0.2.0
		 */
		do_action( "tophive/before-post/{$key}" );
		?>
		 ...
		 * @since 0.2.0
		 */
		do_action( "tophive/after-post/{$key}" );
	}
	...
	function render( $customizer_args = array() ) {
		...
		$atts = array();
		if ( is_numeric( $this->args['columns'] ) && $this->args['columns'] > 1 ) {
			$classes['grid']  = 'tophive-grid-' . $this->args['columns'];
			...
			$classes['grid'] = sprintf( 'tophive-grid-%1$s_sm-%2$s_xs-%3$s', $this->args['columns']['desktop'], $this->args['columns']['tablet'], $this->args['columns']['mobile'] );
		}
		...
		do_action( 'tophive/blog/before-render', $this );
		?>
		<div class="posts-layout-wrapper">
			<div class="<?php echo esc_attr( join( ' ', $classes ) ); ?>"<?php echo tophive_sanitize_filter($s_atts); // WPCS: XSS OK. ?>>
				<?php
		...
		<?php
		do_action( 'tophive/blog/after-render', $this );
	}
	...
	function render_pagination() {
		...
		if ( $this->args['pagination']['show_nav'] ) {
			...
			if ( ! $prev_text ) {
				$prev_text = _x( 'Previous', 'previous set of posts', 'metafans' );
			}
			if ( ! $next_text ) {
				$next_text = _x( 'Next', 'next set of posts', 'metafans' );
			}
		} else {
			$prev_text = false;
			$next_text = false;
