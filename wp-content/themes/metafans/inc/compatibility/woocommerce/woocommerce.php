...
class Tophive_WC {
	...
	function is_active() {
		return tophive_metafans()->is_woocommerce_active();
	}
	...
	function __construct() {
		if ( $this->is_active() ) {
			 ...
			 * Filter shop layout
			 *
			 * @see Tophive_WC::shop_layout
			 */
			add_filter( 'tophive_get_layout', array( $this, 'shop_layout' ) );
			/**
			 * Filter special meta values for shop pages.
			 *
			 * @see Tophive_WC::get_post_metadata
			 */
			add_filter( 'get_post_metadata', array( $this, 'get_post_metadata' ), 999, 4 );

			add_action( 'widgets_init', array( $this, 'register_sidebars' ) );
			add_filter( 'tophive/customizer/config', array( $this, 'customize_shop_sidebars' ) );
			add_filter( 'tophive/sidebar-id', array( $this, 'shop_sidebar_id' ), 15, 2 );

			add_filter( 'tophive_is_header_display', array( $this, 'show_shop_header' ), 15 );
			add_filter( 'tophive_is_footer_display', array( $this, 'show_shop_footer' ), 15 );
			add_filter( 'tophive_site_content_class', array( $this, 'shop_content_layout' ), 15 );
			add_filter( 'tophive_builder_row_display_get_post_id', array( $this, 'builder_row_get_id' ), 15 );

			add_filter( 'tophive/titlebar/args', array( $this, 'titlebar_args' ) );
			add_filter( 'tophive/titlebar/config', array( $this, 'titlebar_config' ), 15, 2 );
			add_filter( 'tophive/titlebar/is-showing', array( $this, 'titlebar_is_showing' ), 15 );
			add_filter( 'tophive/page-header/get-settings', array( $this, 'get_page_header_settings' ) );

			add_filter( 'tophive/theme/js', array( $this, 'add_js' ) );
			...
			// Custom styling.
			add_filter( 'tophive/styling/primary-color', array( $this, 'styling_primary' ) );
			add_filter( 'tophive/styling/secondary-color', array( $this, 'styling_secondary' ) );
			add_filter( 'tophive/styling/link-color', array( $this, 'styling_linkcolor' ) );
			add_filter( 'tophive/styling/color-border', array( $this, 'styling_border_color' ) );
			add_filter( 'tophive/styling/color-meta', array( $this, 'styling_meta_color' ) );

			// Shopping Cart.
			require_once get_template_directory() . '/inc/compatibility/woocommerce/config/header/cart.php';
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'cart_fragments' ) );
			add_filter( 'Metafans_JS', array( $this, 'Metafans_JS' ) );
			...
			add_action( 'tophive_wc_loop_start', array( $this, 'loop_start' ) );
			...
			add_filter( 'woocommerce_product_tabs', array($this, 'tophive_woocommerce_product_tabs') );
			// Catalog config.
			...
			add_filter( 'woocommerce_product_categories_widget_args', array( $this, 'tophive_wc_cat_list_args' ) );
	...
	function tophive_woocommerce_product_tabs( $tabs ){
		global $product;
 
		$tabs['reviews']['title'] = esc_html__('Customer Reviews', 'metafans') . '<span>' . $product->get_review_count() . '</span>';
	...
	function woocommerce_get_script_data( $data, $handle ) {
		if ( 'woocommerce' == $handle ) {
			$data['qty_pm'] = apply_filters( 'tophive_qty_add_plus_minus', 1 );
		}
	...
	function loop_start() {
		...
		wc_set_loop_prop( 'media_secondary', tophive_metafans()->get_setting( 'wc_cd_media_secondary' ) );
		...
		if ( ! $name ) { // Main loop.
			...
			wc_set_loop_prop( 'mobile_columns', tophive_metafans()->get_setting( 'woocommerce_catalog_mobile_columns' ) );

		} elseif ( 'related' == $name || 'up-sells' == $name || 'cross-sells' == $name ) {

			if ( 'up-sells' == $name ) {
				$columns = tophive_metafans()->get_setting( 'wc_single_product_upsell_columns', 'all' );
			} else {
				$columns = tophive_metafans()->get_setting( 'wc_single_product_related_columns', 'all' );
			}
	...
	function related_products_args( $args ) {
		$args['posts_per_page'] = tophive_metafans()->get_setting( 'wc_single_product_related_number' );

		return $args;
	}

	function maybe_disable_related() {
		$n = tophive_metafans()->get_setting( 'wc_single_product_related_number' );
		if ( 0 == $n ) {
	...
	function maybe_disable_upsell() {
		$n = tophive_metafans()->get_setting( 'wc_single_product_upsell_number' );
		if ( 0 == $n ) {
	...
	function updsell_products_args( $args ) {
		$args['posts_per_page'] = tophive_metafans()->get_setting( 'wc_single_product_upsell_number' );
	...
	function custom_styles( $enqueue_styles ) {
		...
		$enqueue_styles['woocommerce-general']['deps'] = 'tophive-style';
	...
	/**
	 * Add more settings to Tophive JS
	 *
	 * @param array $args JS settings.
	 *
	 * @return mixed
	 */
	function Metafans_JS( $args ) {
	...
	function cart_fragments( $cart_fragments = array() ) {
		...
		$cart_fragments['.tophive-wc-sub-total'] = '<span class="tophive-wc-sub-total">' . $sub_total . '</span>';
		...
		$cart_fragments['.tophive-wc-total-qty'] = '<span class="' . $class . '">' . $qty . '</span>';
	...
	function shop_content_layout( $classes = array() ) {
		if ( $this->is_shop_pages() ) {
			$page_layout = $this->get_shop_page_meta( '_tophive_content_layout' );
			$page_spacing = $this->get_shop_page_meta( '_tophive_content_spacing' );
			if ( $page_layout ) {
				$classes['content_layout'] = 'content-' . sanitize_text_field( $page_layout );
			}			
			if ( $page_spacing ) {
				$classes['content_layout'] = 'content-' . sanitize_text_field( $page_spacing );
			}
		}
	...
	function show_shop_header( $show = true ) {
		if ( $this->is_shop_pages() ) {
			$disable = $this->get_shop_page_meta( '_tophive_disable_header' );
			if ( $disable ) {
				$show = false;
			}
		}
	...
	function show_shop_footer( $show = true ) {
		if ( $this->is_shop_pages() ) {
			...
			foreach ( $rows as $row_id ) {
				if ( ! tophive_is_builder_row_display( 'footer', $row_id, $shop_id ) ) {
					$count ++;
				}
			}
			if ( $count >= count( $rows ) ) {
	...
	function show_shop_title( $show = true ) {
		if ( $this->is_shop_pages() ) {
			$disable = $this->get_shop_page_meta( '_tophive_disable_page_title' );
			if ( $disable ) {
				$show = false;
			}
		}
		...
		return apply_filters( 'tophive_is_shop_title_display', $show );
	}
	...
	function get_page_header_settings( $args ) {
		if ( is_product_taxonomy() ) {
			...
			$image        = tophive_metafans()->get_media( $thumbnail_id, 'full' );
			if ( $image ) {
				$args['image'] = $image;
			}
		}
	...
	function titlebar_is_showing( $show = true ) {
		...
		if ( is_shop() ) {
			...
			if ( tophive_metafans()->get_setting( 'titlebar_display_product' ) ) {
				$show = false;
			} else {
				$show = true;
			}
			if ( tophive_metafans()->is_using_post() ) {
				$breadcrumb_display = get_post_meta( wc_get_page_id( 'shop' ), '_tophive_breadcrumb_display', true );
			...
			if ( tophive_metafans()->get_setting( 'titlebar_display_product_tax' ) ) {
				$show = true;
			} else {
				$show = false;
			}
	...
	function titlebar_config( $config, $titlebar ) {
		...
		$config[] = array(
			...
			'default'        => 1,
			'section'        => $section,
			'checkbox_label' => esc_html__( 'Display on product taxonomies (categories/tags,..)', 'metafans' ),
		);
		...
		$config[] = array(
			...
			'default'        => 1,
			'section'        => $section,
			'checkbox_label' => esc_html__( 'Display on single product', 'metafans' ),
		);
	...
	function register_sidebars() {
		register_sidebar(
			array(
				'name'          => esc_html__( 'WooCommerce Primary Sidebar', 'metafans' ),
				'id'            => 'shop-sidebar-1',
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'WooCommerce Secondary Sidebar', 'metafans' ),
				'id'            => 'shop-sidebar-2',
				'description'   => esc_html__( 'Add widgets here.', 'metafans' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
	...
	function get_post_metadata( $value, $object_id, $meta_key, $single ) {
		$meta_keys = array(
			'_tophive_page_header_display' => '',
			'_tophive_breadcrumb_display'  => '',
		);
		...
		if ( wc_get_page_id( 'cart' ) == $object_id || wc_get_page_id( 'checkout' ) == $object_id ) {
			...
			switch ( $meta_key ) {
				case '_tophive_page_header_display':
					if ( empty( $value ) || 'default' == $value[0] || 'normal' == $value[0] || ! $value[0] ) {
						$value[0] = 'normal';
					}
					break;
				case '_tophive_breadcrumb_display':
	...
	function shop_layout( $layout = false ) {
		if ( $this->is_shop_pages() ) {
			...
			$page_custom = get_post_meta( $page_id, '_tophive_sidebar', true );
			if ( $page_custom ) {
				$layout = $page_custom;
			} elseif ( $page ) {
				$layout = $page;
		...
		if ( is_product() ) {
			$product_sidebar = get_post_meta( get_the_ID(), '_tophive_sidebar', true );
			if ( ! $product_sidebar ) {
				$product_custom = tophive_metafans()->get_setting( 'product_sidebar_layout' );
...
function Tophive_WC() {
	return Tophive_WC::get_instance();
}
 ...
 * @return string
 */
function tophive_get_default_catalog_view_mod() {
	$name    = wc_get_loop_prop( 'name' );
	$default = tophive_metafans()->get_setting( 'wc_cd_default_view' );
	if ( $name ) {
		return apply_filters( 'tophive_get_default_catalog_view_mod', 'grid' );
	}
	...
	if ( ! tophive_metafans()->get_setting( 'wc_cd_show_view_mod' ) ) {
		$use_cookies = false;
	}

	if ( $use_cookies ) { // Do not use cookie in customize.
		$cookie_mod = ( isset( $_COOKIE['tophive_wc_pl_view_mod'] ) && $_COOKIE['tophive_wc_pl_view_mod'] ) ? sanitize_text_field( $_COOKIE['tophive_wc_pl_view_mod'] ) : false; // WPCS: sanitization ok.
		if ( $cookie_mod ) {
	...
	return apply_filters( 'tophive_get_default_catalog_view_mod', $default );
}
