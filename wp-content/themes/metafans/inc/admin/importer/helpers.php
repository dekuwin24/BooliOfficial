<?php
/**
 * Helper Class used in the theme
 *
 * @package MasterCLass
 */
require_once 'import-downloader.php';
class Helpers {
	/**
	...
	public static function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( self::is_import_file_inmf_format_correct( $import_file ) ) {
	 ...
	 * @return boolean
	 */
	private static function is_import_file_inmf_format_correct( $import_file_info ) {
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'metafans' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'metafans' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'metafans' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import' , 'metafans' ),
				'menu_title'  => esc_html__( 'Import Demo Data' , 'metafans' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'metafans' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'metafans' )
			);
		}
	...
	public static function get_log_path() {
		...
		$upload_path = apply_filters( 'tophive/demo-import/upload/file-path', trailingslashit( $upload_dir['path'] ) );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'metafans' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		check_ajax_referer( 'tophive_mf_ajax_verification', 'security' );

		// Check if user has the WP capability to import data.
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					esc_html__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'metafans' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$file_not_provided_error = array(
			'error' => esc_html__( 'No file provided.', 'metafans' )
		);
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					esc_html__( 'Content file was not uploaded. Error: %s', 'metafans' ),
					$content_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'metafans' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					esc_html__( 'Widget file was not uploaded. Error: %s', 'metafans' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					esc_html__( 'Customizer file was not uploaded. Error: %s', 'metafans' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				...
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'metafans' ),
					$log_file_path,
					esc_html__( 'Upload files', 'metafans' )
				);
			}
			...
			$log_added = self::append_to_file(
				sprintf(
					esc_html__( 'Redux file was not uploaded. Error: %s', 'metafans' ),
		...
		$log_added = self::append_to_file(
			esc_html__( 'The import files were successfully uploaded!', 'metafans' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'metafans' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'metafans' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'metafans' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'metafans' ) : $selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'metafans' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'metafans' ) : $selected_import_files['customizer'],
			empty( $redux_file_string ) ? esc_html__( 'not defined!', 'metafans' ) : $redux_file_string
		);
	}
	 ...
	 */
	public static function set_tophive_import_data_transient( $data ) {
		set_transient( 'tophive_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}
}
