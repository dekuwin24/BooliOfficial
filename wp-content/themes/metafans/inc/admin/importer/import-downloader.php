...
class Downloader {
	...
	private function get_content_from_url( $url ) {
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				esc_html__( 'Missing Download URL!', 'metafans' )
			);
		}
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					esc_html__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'metafans' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
				...
				apply_filters( 'tophive/demo-import/download/file-fetch-error', '' )
			);
		}
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = apply_filters( 'tophive/demo-import/upload/file-path', trailingslashit( $upload_dir['path'] ) );
		}
	}
}
