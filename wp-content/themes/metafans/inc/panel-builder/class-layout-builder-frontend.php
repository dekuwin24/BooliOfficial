 ...
 * @since 0.2.9
 */
class Tophive_Layout_Builder_Frontend extends Tophive_Abstract_Layout_Frontend {
	...
	function get_settings() {
		...
		$data = tophive_metafans()->get_setting( $this->control_id );
		$data = wp_parse_args(
			$data,
			array(
				'desktop' => '',
				...
				if ( is_array( $row_items ) && count( $row_items ) ) {
					...
					foreach ( $row_items as $item_index => $item ) {
						...
						if ( ! $items[ $item['id'] ] ['render_content'] ) {
							 ...
							 * @since 0.2.1
							 */
							do_action( 'tophive/builder/' . $this->id . '/before-item/' . $item['id'] );
							$object_item = Tophive_Customize_Layout_Builder()->get_builder_item( $this->id, $item['id'] );
							 ...
							 * @since 0.2.1
							 */
							do_action( 'tophive/builder/' . $this->id . '/after-item/' . $item['id'] );
	...
	public function render_row( $items, $id = '', $device = 'desktop' ) {
		...
		while ( $index < $n ) {
			...
			$merge      = tophive_metafans()->get_setting( $merge_key, $device );
			$merge_next = false;
			$merge_prev = false;
			if ( 'no' == $merge || '0' == $merge ) {
				$merge = false;
			...
			if ( $next_item ) {
				...
				$merge_next     = tophive_metafans()->get_setting( $merge_key_next, $device );
			}
		...
		foreach ( $group_items as $item ) {
			...
			if ( 'footer' != $this->id ) {
				$classes[] = "tophive-col-{$width}_md-{$width}_sm-{$width}";
			} else {
				if ( $number_group_item > 1 ) {
					$classes[] = "tophive-col-{$width}_md-{$width}_sm-6_xs-12";
				} else {
					$classes[] = "tophive-col-{$width}_md-{$width}_sm-12_xs-12";
				}
			}
			...
			$classes = apply_filters( 'tophive/builder/item-wrapper-classes', $classes, $item );
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_items ) || ! empty( $mobile_items ) ) {
						...
						if ( 'footer' != $this->id ) {
							...
							$align_classes = 'tophive-grid-top';
						}
	 ...
	 * Render sidebar row
	 */
	public function render_mobile_sidebar( $compatibility = '' ) {
		$id                = 'sidebar';
		...
		if ( ! empty( $mobile_items ) || is_customize_preview() ) {
			...
			echo tophive_sanitize_filter($compatibility);
 ...
 * @deprecated 0.2.7
 */
class Tophive_Customize_Layout_Builder_Frontend extends Tophive_Layout_Builder_Frontend {}

/**
 * Alias of class Tophive_Layout_Builder_Frontend
 *
 * @see Tophive_Layout_Builder_Frontend
 *
 * @return Tophive_Layout_Builder_Frontend
 */
function Tophive_Layout_Builder_Frontend() {
	return Tophive_Layout_Builder_Frontend::get_instance();
}
 ...
 * @return Tophive_Layout_Builder_Frontend
 */
function Tophive_Customize_Layout_Builder_Frontend() {
	return Tophive_Layout_Builder_Frontend();
}
