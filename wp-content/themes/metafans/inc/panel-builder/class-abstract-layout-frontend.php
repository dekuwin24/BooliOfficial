 ...
 * @since 0.2.9
 */
abstract class Tophive_Abstract_Layout_Frontend {
	...
	public function close_icon( $class = '' ) {
		$menu_sidebar_skin = tophive_metafans()->get_setting( 'header_sidebar_text_mode' );
		$close             = '<a class="close is-size-medium ' . $menu_sidebar_skin . esc_attr( $class ) . '" href="#">
        <span class="hamburger hamburger--squeeze">
            <span class="hamburger-box">
              <span class="hamburger-inner"><span class="screen-reader-text">' . esc_html__( 'Menu', 'metafans' ) . '</span></span>
            </span>
        </span>
        <span class="screen-reader-text">' . esc_html__( 'Close', 'metafans' ) . '</span>
        </a>';
