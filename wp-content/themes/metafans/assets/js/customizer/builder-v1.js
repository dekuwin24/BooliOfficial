		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".tophive--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".tophive--cb-items", panel).each(function(index) {
					...
					$(
						".tophive-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
						...
						start: function(event, ui) {
							...
							$(".tophive--cb-items", panel).css("z-index", "");
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".tophive--device-panel.tophive--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".tophive--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-tophive--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="tophive-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.tophive-available-items[data-device="' +
										device +
