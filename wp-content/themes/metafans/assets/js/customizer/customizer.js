...
(function($, api) {
	...
	var header_changed = function(partial_id, remove_items) {
		...
		if (
			...
			partial_id === "tophive_customize_render_header"
		) {
	...
	api.bind("preview-ready", function() {
		...
		function setupPreviewNamePosition() {
			$(".tophive-grid .has_menu.builder-item-focus").each(function() {
				var parentPos = $(this)
					.closest(".tophive-grid")
					.offset();
	...
	var skips_to_add_shortcut = {
		tophive_customize_render_header: 1,
		tophive_customize_render_footer: 1
	};
